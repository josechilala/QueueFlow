...
export async function getServerSession(): Promise<{ status: number; retryAfter?: string; user?: AuthenticatedUser }> {
  const token = (await cookies()).get(ACCESS_COOKIE)?.value;
  if (!token) return { status: 401 };
  const response = await sessionFetch(`${apiUrl}/api/v1/auth/me`, { headers: { Authorization: `Bearer ${token}` }, cache: 'no-store' });
  if (!response.ok) return { status: response.status, retryAfter: response.headers.get('Retry-After') ?? undefined };
