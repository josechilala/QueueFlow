...
describe('public scheduling link', () => {
  ...
  it('usa a base configurada e o slug da organização autenticada', () => {
    expect(buildPublicSchedulingUrl('https://agenda.queueflow.com/', 'clinica-vida')).toBe('https://agenda.queueflow.com/agendamento/clinica-vida');
    expect(buildPublicSchedulingUrl('https://agenda.queueflow.com', 'empresa-b')).toBe('https://agenda.queueflow.com/agendamento/empresa-b');
  });
  ...
  it('copia exatamente a URL pública', async () => {
    ...
    await copyPublicSchedulingUrl('https://agenda.queueflow.com/agendamento/clinica-vida', { writeText });
    expect(writeText).toHaveBeenCalledWith('https://agenda.queueflow.com/agendamento/clinica-vida');
  });

  it('propaga falha da Clipboard API para a interface exibir feedback', async () => {
    const writeText = vi.fn().mockRejectedValue(new Error('clipboard denied'));
    await expect(copyPublicSchedulingUrl('https://agenda.queueflow.com/agendamento/clinica-vida', { writeText })).rejects.toThrow('clipboard denied');
  });
});
