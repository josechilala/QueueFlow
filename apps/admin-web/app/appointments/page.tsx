...
export default async function AppointmentsPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  return <AdminShell user={session.user}>
    ...
    <div className="table-wrap"><table><thead><tr><th>Atendimento</th><th>Cliente</th><th>Serviço</th><th>Unidade</th><th>Origem</th><th>Criado em</th><th>Status</th><th /></tr></thead><tbody>{appointments.data.map(item => <tr key={item.id}><td>{new Date(item.scheduledStart).toLocaleString('pt-BR')}</td><td><strong>{item.customerName}</strong><div className="muted">{item.customerPhone ?? item.customerEmail ?? '—'}</div></td><td>{item.serviceName}</td><td>{item.branchName}</td><td>{appointmentOriginLabel[item.origin]}</td><td>{new Date(item.createdAt).toLocaleString('pt-BR')}</td><td><span className={`status appointment-${item.status.toLowerCase()}`}>{labels[item.status]}</span></td><td><Link href={`/appointments/${item.id}`}>Detalhes</Link></td></tr>)}</tbody></table></div>
  </AdminShell>;
}
