...
export default async function WorkstationPage() {
  ...
  return <main className="shell"><RealtimeRefresh queuePublicIds={operation.data.queues.map(queue => queue.publicId)} /><header><div><div className="brand">QueueFlow</div><h1>Atendimento</h1><p>{session.data.name} · {session.data.role}</p></div><form action="/api/auth/logout" method="post"><button className="secondary">Sair</button></form></header><Workstation context={operation.data} /></main>;
}
