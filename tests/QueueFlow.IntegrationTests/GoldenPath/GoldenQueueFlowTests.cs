...
public sealed class GoldenQueueFlowTests : IClassFixture<QueueFlowApiFactory>
{
    ...
    public async Task MainProductFlowWorksEndToEnd()
    {
        ...
        try
        {
            ...
            var checkIn = await client.PostAsJsonAsync($"/api/v1/public/appointments/{appointment.PublicToken}/check-in", new { }, ct); Assert.Equal(HttpStatusCode.OK, checkIn.StatusCode); var checkedIn = await checkIn.Content.ReadFromJsonAsync<PublicAppointment>(ct); Assert.NotNull(checkedIn); Assert.Equal("CheckedIn", checkedIn.Status); Assert.NotNull(checkedIn.QueueTicketToken);
