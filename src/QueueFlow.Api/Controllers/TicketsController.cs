...
[ApiController, Authorize(Policy = "AttendantPanel"), Route("api/v1/tickets")]
public sealed class TicketsController(QueueOperationsService service) : ControllerBase
{
    ...
    [HttpPost("{id:guid}/recall")] public async Task<IActionResult> Recall(Guid id, CancellationToken ct) => ToActionResult(await service.RecallAsync(id, ct));
